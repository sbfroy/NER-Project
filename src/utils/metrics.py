...
class SpanAcc(Metric): # 
    ...
    def update(self, preds: torch.Tensor, targets: torch.Tensor):
        ...
        for pred, target in zip(preds, targets):
            pred_spans = self.extract_spans(pred.tolist())
            gold_spans = self.extract_spans(target.tolist())

            self.correct_spans += len(pred_spans & gold_spans)
            self.gold_spans += len(gold_spans)

    def compute(self):
        return self.correct_spans / self.gold_spans if self.gold_spans > 0 else torch.tensor(0.0)
